...
class Dytt:
    ...
    def parse_movie_list(self,consumer):
        next(consumer)
        for index in range(1,4):
            ...
            if (len(results_movie_name) + len(results_movie_rate) == 0):
                ...
                for result_detail in results:
                    ...
                    result_final = consumer.send(self.parse_movie_detail(response_detail))
    ...
    def run(self):
        result_final = list()
        while True:
            n = yield result_final
            if not n:
                return
            self.save_data(n)

if __name__ == '__main__':
    dytt = Dytt(str(MovieType.Comedy))
    c = dytt.run()
    dytt.parse_movie_list(c)